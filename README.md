...
MVP code complete end-to-end for a Bower-shaped chat enclave: wire protocol (Noise XX + protobuf), CLI (`tabula enclave {up,down,status,ssh}` + `chat` + `keygen`), GCP OpenTofu modules, audit + cost guardrails, GPU bootstrap. Tested locally; not yet deployed.
...
| [`terraform/`](terraform/) | GCP modules + root enclave composition (driven by [OpenTofu](https://opentofu.org/); `terraform` works as a fallback) |
